...
// Game states (now managed by GameEngine)
...
function addExp(userId, amount = 10) {
...
// Utility functions removed - now using UtilityManager
...
const commands = {
    ...
    async removeapikey2(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['removeapikey2', ...args], msg);
    },

    async resetapiusage(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['resetapiusage', ...args], msg);
    },

    async removedailylimit(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['removedailylimit', ...args], msg);
    },

    async aihistory(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['aihistory', ...args], msg);
    },
    
    async checkai(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['checkai', ...args], msg);
    },
    
    async botperformance(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['botperformance', ...args], msg);
    },
    
    async systemhealth(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['systemhealth', ...args], msg);
    },
    
    async adminstats(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['adminstats', ...args], msg);
    },
    
    async errorlogs(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['errorlogs', ...args], msg);
    },
    
    async addquiz(args, msg) {
        if (!adminSystem.isAdmin(msg.author)) {
            return '❌ Kamu bukan admin!';
        }
        return await adminSystem.handleCommand(['addquiz', ...args], msg);
    },
...
client.on('message_create', async (msg) => {
    ...
    if (gameEngine.quizStates.has(msg.from)) {
        ...
        if (['1', '2', '3', '4'].includes(answer)) {
            ...
            if (selectedOption === state.question.a) {
...
client.on('group_leave', async (notification) => {
    const chat = await notification.getChat();
    const goodbyeMsg = botConfig.languages.id.goodbye;
    chat.sendMessage(goodbyeMsg);
});
...
cron.schedule('* * * * *', async () => {
    try {
        ...
        for (const reminder of reminders) {
            const chatId = reminder.group_id || reminder.user_id;
...
console.log('🚀 Starting WhatsApp Bot for VPS deployment...');
console.log('📍 Bot akan berjalan tanpa dashboard web');
